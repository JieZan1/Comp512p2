...
public class Paxos implements GCDeliverListener {
    ...
    public Paxos(String myProcess, String[] allGroupProcesses, Logger logger, FailCheck failCheck)
        ...
        this.processIndex = Arrays.asList(allGroupProcesses).indexOf(myProcess);
    ...
    public void broadcastTOMsg(Object val) {
        ...
        if (!isLeader()) {
            gcl.sendMsg(encapsulated_val, allGroupProcesses[0]);
            return;
        }
    ...
    public void deliver(String sender, Object msg) {
        // if leader receives a propose request from another process
        if (msg instanceof LabelObj && isLeader()) {
            int seq = currentSequence.getAndIncrement();
            LabelObj val = (LabelObj) msg;
            PendingValue pv = new PendingValue(val);
            pendingValues.put(seq, pv);
            proposeValue(seq, val, pv);
            return;
        }
    ...
    private void handlePromise(String sender, PromiseMessage msg) {
        ...
        synchronized (instance) {
            ...
            if (instance.promiseCount >= MAJORITY) {
                logger.fine(
                        "Reached majority promises for seq=" + msg.sequence + " (count=" + instance.promiseCount + ")");
